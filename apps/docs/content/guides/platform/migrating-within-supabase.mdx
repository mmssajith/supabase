...
- **Project migration only migrates the Postgres database** and anything else will need to be re-configured.
...
Here are some items to note down your changes made to re-configure in the new project:
...
If you already have the database backup, skip to the Restore project step.
...
<StepHikeCompact>
    <StepHikeCompact.Step step={1}>
        <StepHikeCompact.Details title="Create and configure new project" fullWidth>
    ...
    <StepHikeCompact.Step step={2}>
      <StepHikeCompact.Details title="Get the new Session pooler connection string" fullWidth>
      Get your Session pooler connection string from your [Connect page](https://supabase.com/dashboard/project/_?showConnect=true). Example connection string:
      ```bash
        postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@aws-0-[PROJECT-REGION].pooler.supabase.com:5432/postgres
      ```
    ...
    <StepHikeCompact.Step step={5}>
      <StepHikeCompact.Details title="Restore your Project with the CLI" fullWidth>
          Replace ```[CONNECTION_STRING]``` with your connection string from the previous steps and run the following command:
...
The new project has the old project's Storage buckets, but the Storage objects need to be migrated manually. If you have your storage objects locally (eg 90 day paused projects), migrate the storage objects to the new project using this [google colab script](https://colab.research.google.com/github/PLyn/supabase-storage-migrate/blob/main/Supabase_Storage_migration.ipynb).
...
>
    <div className="border-b mt-3 pb-3">
        <AccordionItem
        header={<span className="text-foreground">Storage migration script</span>}
        id="backup-project"
        >
        ```js
